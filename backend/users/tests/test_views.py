...
from users.forms import ContactUsForm, UserLoginForm, UserRegistrationForm
...
class RegistrationPages(TestCase):
    ...
    def test_register_page_valid_form(self):
        ...
        response = self.client.post(path=reverse('users:sign_up'), data=form_data)
        self.assertRedirects(response, expected_url=reverse('users:register_done'))
    ...
    def test_register_page_invalid_form(self):
        ...
        response = self.client.post(path=reverse('users:sign_up'), data=form_invalid_data)
    ...
    def test_register_done_page_user_correct_template(self):
        response = self.client.get(reverse('users:register_done'))
...
class LoginPage(TestCase):
    ...
    def test_login_page_redirect_to_home_page(self):
        ...
        response = self.client.post(path=reverse('users:login'), data=user_data)
...
class ProfilePage(TestCase):
    ...
    def test_profile_page_redirects_anonymous_users(self):
        ...
        self.assertRedirects(response, expected_url=reverse('users:login') + f"?next={reverse('users:profile')}")
...
class PasswordRecoveryPage(TestCase):
    ...
    def test_password_recovery_page_redirect_to_correct_page(self):
        ...
        response = self.client.post(path=reverse('users:password_reset'), data=email_for_recovery)
        self.assertRedirects(response, expected_url=reverse('users:password_reset_done'))
...
class PasswordChangePage(TestCase):
    ...
    def test_password_change_page_redirects_for_annonymous_users(self):
        response = self.client.get(reverse('users:password_change'))
        self.assertRedirects(response, expected_url=reverse('users:login') + f"?next={reverse("users:password_change")}")
    ...
    def test_password_change_page_valid_form(self):
        ...
        response = self.client.post(path=reverse('users:password_change'), data=form_data)
        self.assertRedirects(response, expected_url=reverse('users:password_change_done'))
...
class ContactUsPage(TestCase):
    ...
    def test_contact_page_redirects_for_anonymous_users(self):
        ...
        self.assertRedirects(response, expected_url=reverse('users:login') + f"?next={reverse("contact")}")
